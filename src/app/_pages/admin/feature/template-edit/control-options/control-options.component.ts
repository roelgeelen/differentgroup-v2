...
export class ControlOptionsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.templateService.getDealSchema().subscribe(schema => {
  ...
  prepareFilesList(control: IFormControl, files: FileList) {
    ...
    if (file !== null && file.size < 5000000) {
      this.templateService.upload(this.formService.form$.getValue().id!.toString(), control.id, file).subscribe({
        next: (data: any) => {
  ...
  removeImage(control: IFormControl, image: IFormAttachment) {
    this.templateService.removeFormAttachment(this.formService.form$.getValue().id!.toString(), image.id).subscribe(r => {
  ...
  saveForm() {
    ...
    this.templateService.saveTemplate(this.formService.form$.getValue()).subscribe(f => {
  ...
  hubspotSelected($event: any, control: IFormControl) {
    ...
    if (field !== undefined) {
      ...
      if (field?.options.length > 0 && control.options?.choices !== undefined) {
        Swal.fire({
