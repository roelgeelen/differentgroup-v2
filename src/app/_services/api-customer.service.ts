...
export class ApiCustomerService {
  ...
  getConfigurations(id: string) {
    return this.http.get<IConfiguration[]>(`${environment.apiUrl}/v2/customer/${id}/configurations`);
  }

  createConfiguration(id: string, config: IConfiguration) {
    return this.http.post<IConfiguration>(`${environment.apiUrl}/v2/customer/${id}/configurations`, config);
  }

  getConfiguration(id: string, configId: string) {
    return this.http.get<IConfiguration>(`${environment.apiUrl}/v2/customer/${id}/configurations/${configId}`);
  }

  updateConfiguration(id: string, configId: string, config: IConfiguration) {
    return this.http.put(`${environment.apiUrl}/v2/customer/${id}/configurations/${configId}`, config);
  }

  upload(id:string, configId: string, field: string, file: File){
    const formData: FormData = new FormData();
    formData.append('file', file);
    return this.http.post<IFormAttachment>(`${environment.apiUrl}/v2/customer/${id}/configurations/${configId}/fields/${field}/attachments`, formData, {
      reportProgress: true,
      observe: 'events'
    });
  }
  ...
  deleteConfiguration(id: string, configId: string) {
    return this.http.delete(`${environment.apiUrl}/v2/customer/${id}/configurations/${configId}`);
  }
}
